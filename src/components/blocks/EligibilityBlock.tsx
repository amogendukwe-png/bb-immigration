...
export const EligibilityBlock: React.FC = () => {
  ...
  const renderPassportEligibility = () => (
    <>
      <div className={`p-4 ${formErrors.age ? 'border-l-4 border-[#d4351c]' : ''}`}>
        <fieldset className="space-y-4">
          <legend className="text-xl font-bold mb-2">How old is the person the passport is for?</legend>
          {formErrors.age && <span className="text-[#d4351c] font-bold block mb-2">{formErrors.age}</span>}
          <div className="flex flex-col gap-3">
            <label className="flex items-center gap-3 cursor-pointer group p-3 border-2 border-transparent hover:border-[#005ea5] transition-all">
              <input
                type="radio" name="age" className="w-8 h-8 cursor-pointer accent-[#005ea5]"
      ...
      <div className={`p-4 ${formErrors.isBarbadosCitizen ? 'border-l-4 border-[#d4351c]' : ''}`}>
        <fieldset className="space-y-4">
          <legend className="text-xl font-bold mb-2">Is the applicant a Citizen of Barbados?</legend>
          {formErrors.isBarbadosCitizen && <span className="text-[#d4351c] font-bold block mb-2">{formErrors.isBarbadosCitizen}</span>}
          <div className="flex flex-col gap-3">
            <label className="flex items-center gap-3 cursor-pointer group p-3 border-2 border-transparent hover:border-[#005ea5] transition-all">
              <input
                type="radio" name="isCitizen" className="w-8 h-8 cursor-pointer accent-[#005ea5]"
  ...
  const renderWorkPermitEligibility = () => (
    <div className={`p-4 ${formErrors.subType ? 'border-l-4 border-[#d4351c]' : ''}`}>
      <fieldset className="space-y-4">
        <legend className="text-xl font-bold mb-2">Select the type of work permit you are applying for</legend>
        {formErrors.subType && <span className="text-[#d4351c] font-bold block mb-2">{formErrors.subType}</span>}
        <div className="flex flex-col gap-3">
          <label className="flex items-center gap-3 cursor-pointer group p-3 border-2 border-transparent hover:border-[#005ea5] transition-all">
            <input
              type="radio" name="subType" className="w-8 h-8 cursor-pointer accent-[#005ea5]"
      ...
      <fieldset className="space-y-4">
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            <label key={opt.id} className="flex items-center gap-3 cursor-pointer group p-3 border-2 border-transparent hover:border-[#005ea5] transition-all">
              <input
                type="radio" name="subType" className="w-8 h-8 cursor-pointer accent-[#005ea5]"
  ...
  const renderGenericEligibility = () => {
    ...
    return (
      <div className="p-4 bg-[#f3f2f1] border-l-4 border-[#005ea5]">
  ...
  return (
    <div className="space-y-8" id="eligibility-block">
      ...
      <h1 className="text-3xl md:text-4xl font-bold mb-8 underline decoration-2 decoration-[#005ea5] underline-offset-8">Eligibility</h1>
      ...
      <form onSubmit={validateAndSubmit} className="space-y-12">
        ...
        <button
          type="submit"
          className="bg-[#00703c] text-white px-8 py-3 rounded-sm font-bold text-xl hover:bg-[#005a30] active:translate-y-1 transition-all shadow-md focus:outline-none focus:ring-4 focus:ring-[#ffdd00]"
          id="continue-button"
        >
          Continue
        </button>
