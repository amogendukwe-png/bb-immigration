...
export const ContactBlock: React.FC = () => {
  ...
  return (
    <div className="space-y-8" id="contact-block">
      ...
      <h1 className="text-3xl md:text-4xl font-bold mb-8 underline decoration-2 decoration-[var(--bb-navy-mid)] underline-offset-8 tracking-tight">Contact details</h1>
      ...
      <form onSubmit={validateAndSubmit} className="space-y-10 max-w-2xl">
        <div className={`flex flex-col gap-2 ${formErrors.telLocal ? 'border-l-4 border-[var(--bb-red)] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="telLocal">Telephone number (local)</label>
          <span className="text-[var(--bb-text-gray)]">For example, 246 123 4567</span>
          <input
            id="telLocal"
            type="tel"
            className="border-2 border-black p-3 text-xl w-full max-w-[400px] focus:ring-4 focus:ring-[var(--bb-focus)] focus:outline-none"
            placeholder="Local Barbados number"
          />
        </div>

        <div className={`flex flex-col gap-2 ${formErrors.email ? 'border-l-4 border-[var(--bb-red)] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="email">Email address</label>
          <span className="text-[var(--bb-text-gray)]">We will send your confirmation and case status to this address</span>
          {formErrors.email && <span className="text-[var(--bb-red)] font-bold">{formErrors.email}</span>}
          <input
            id="email"
            type="email"
            className="border-2 border-black p-3 text-xl w-full focus:ring-4 focus:ring-[var(--bb-focus)] focus:outline-none"
            value={details.email || ''}
