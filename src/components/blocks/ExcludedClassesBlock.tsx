...
export const ExcludedClassesBlock: React.FC = () => {
  ...
  return (
    <div className="space-y-8" id="excluded-classes-block">
      ...
      <div className="p-6 bg-[var(--color-white-00)] border-l-4 border-[var(--color-teal-00)] mb-12 max-w-3xl">
      ...
      <form onSubmit={(e) => { e.preventDefault(); goToNext(); }} className="space-y-12">
        <fieldset className="space-y-4">
          ...
          <div className="space-y-4 max-w-4xl">
            {classes.map((cls) => (
              <label key={cls.id} className="flex items-start gap-4 p-4 border-2 border-[var(--color-black-00)] hover:border-[var(--color-teal-00)] cursor-pointer transition-all bg-white group">
                <input
                  type="checkbox"
                  className="w-8 h-8 mt-1 accent-[var(--color-teal-00)] cursor-pointer"
        ...
        <div className="pt-8 border-t border-[var(--color-black-00)] flex items-center justify-between max-w-4xl">
          <p className="text-[var(--color-mid-grey-00)] max-w-md italic">
