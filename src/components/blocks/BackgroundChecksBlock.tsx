...
export const BackgroundChecksBlock: React.FC = () => {
  ...
  return (
    <div className="space-y-8" id="background-checks-block">
      ...
      <p className="text-xl mb-4 opacity-80 leading-relaxed max-w-2xl text-[var(--color-red-00)] font-bold italic">
      ...
      <form onSubmit={(e) => { e.preventDefault(); goToNext(); }} className="space-y-12 max-w-3xl">
          ...
          <div key={q.id} className="space-y-4">
            <fieldset className="space-y-4">
              ...
              <div className="flex gap-6 mt-2">
                <label className="flex items-center gap-3 cursor-pointer p-2 border-2 border-transparent hover:border-[var(--color-teal-00)]">
                  <input
                    type="radio" name={q.id} className="w-8 h-8 accent-[var(--color-teal-00)]"
            ...
            {background[q.id as keyof typeof background] === true && (
              <div className="p-4 border-l-4 border-[var(--color-teal-00)] bg-[var(--color-white-00)] animate-in fade-in slide-in-from-top-1 duration-300">
                ...
                <textarea
                  className="w-full border-2 border-black p-4 text-lg focus:ring-4 focus:ring-[var(--color-teal-100)] focus:outline-none"
                  rows={3}
        ...
        <div className="pt-8 border-t border-[var(--color-black-00)]">
          <button
            type="submit"
            className="bg-[var(--color-teal-00)] text-white px-10 py-4 rounded-sm font-bold text-2xl hover:bg-[var(--color-blue-00)] transition-all shadow-md focus:ring-4 focus:ring-[var(--color-teal-100)] focus:outline-none"
          >
            Save and continue
          </button>
        </div>
