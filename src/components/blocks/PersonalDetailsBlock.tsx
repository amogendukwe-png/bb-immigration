  ...
  return (
    <div className="space-y-8" id="personal-details-block">
      ...
      <form onSubmit={validateAndSubmit} className="space-y-10 max-w-2xl">
        <div className={`flex flex-col gap-2 ${formErrors.lastName ? 'border-l-4 border-[#d4351c] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="lastName">Family name (Surname)</label>
          <span className="text-[#505a5f]">In capital letters, e.g. BRATHWAITE</span>
          {formErrors.lastName && <span className="text-[#d4351c] font-bold">{formErrors.lastName}</span>}
          <input
            id="lastName"
            className="border-2 border-black p-2 text-xl w-full uppercase focus:ring-4 focus:ring-[#ffdd00] focus:outline-none"
            value={details.lastName || ''}
            onChange={(e) => handleUpdate('lastName', e.target.value)}
          />
        </div>

        <div className={`flex flex-col gap-2 ${formErrors.firstName ? 'border-l-4 border-[#d4351c] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="firstName">First and middle names</label>
          <span className="text-[#505a5f]">In capital letters, as they appear on the birth certificate</span>
          {formErrors.firstName && <span className="text-[#d4351c] font-bold">{formErrors.firstName}</span>}
          <input
            id="firstName"
            className="border-2 border-black p-2 text-xl w-full uppercase focus:ring-4 focus:ring-[#ffdd00] focus:outline-none"
        ...
        <div className={`flex flex-col gap-2 ${formErrors.dateOfBirth ? 'border-l-4 border-[#d4351c] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="dob">Date of birth</label>
          <span className="text-[#505a5f]">For example, 31 3 1980</span>
          {formErrors.dateOfBirth && <span className="text-[#d4351c] font-bold">{formErrors.dateOfBirth}</span>}
          <input
            id="dob"
            type="date"
        ...
        <div className={`flex flex-col gap-2 ${formErrors.sex ? 'border-l-4 border-[#d4351c] pl-4' : ''}`}>
          <fieldset className="space-y-4">
            <legend className="text-xl font-bold" id="sex">Sex</legend>
            {formErrors.sex && <span className="text-[#d4351c] font-bold block">{formErrors.sex}</span>}
            <div className="flex gap-6">
              {['MALE', 'FEMALE'].map((s) => (
                <label key={s} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio" name="sex" className="w-6 h-6 cursor-pointer accent-[#005ea5]"
