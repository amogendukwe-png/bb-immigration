  ...
  return (
    <div className="space-y-8" id="personal-details-block">
      ...
      <form onSubmit={validateAndSubmit} className="space-y-10 max-w-2xl">
        <div className={`flex flex-col gap-2 ${formErrors.lastName ? 'border-l-4 border-[var(--color-red-00)] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="lastName">Family name (Surname)</label>
          <span className="text-[var(--color-mid-grey-00)]">In capital letters, e.g. BRATHWAITE</span>
          {formErrors.lastName && <span className="text-[var(--color-red-00)] font-bold">{formErrors.lastName}</span>}
          <input
            id="lastName"
        ...
        <div className={`flex flex-col gap-2 ${formErrors.firstName ? 'border-l-4 border-[var(--color-red-00)] pl-4' : ''}`}>
          <label className="text-xl font-bold" htmlFor="firstName">First and middle names</label>
          <span className="text-[var(--color-mid-grey-00)]">In capital letters, as they appear on the birth certificate</span>
          {formErrors.firstName && <span className="text-[var(--color-red-00)] font-bold">{formErrors.firstName}</span>}
          <input
            id="firstName"
            className="border-2 border-black p-2 text-xl w-full uppercase focus:ring-4 focus:ring-[var(--color-teal-100)] focus:outline-none"
        ...
        <div className={`flex flex-col gap-2 ${formErrors.dateOfBirth ? 'border-l-4 border-[var(--color-red-00)] pl-4' : ''}`}>
          ...
          <span className="text-[var(--color-mid-grey-00)]">For example, 31 3 1980</span>
        ...
        <div className={`flex flex-col gap-2 ${formErrors.sex ? 'border-l-4 border-[var(--color-red-00)] pl-4' : ''}`}>
          <fieldset className="space-y-4">
            <legend className="text-xl font-bold" id="sex">Sex</legend>
            {formErrors.sex && <span className="text-[var(--color-red-00)] font-bold block">{formErrors.sex}</span>}
            <div className="flex gap-6">
              {['MALE', 'FEMALE'].map((s) => (
                <label key={s} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio" name="sex" className="w-6 h-6 cursor-pointer accent-[var(--color-teal-00)]"
